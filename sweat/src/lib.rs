...
pub struct Contract {
    ...
    token: FungibleToken,
    token_imitation: LookupMap<AccountId, Balance>,
}
...
impl Contract {
    ...
    pub fn new(oracles_vec: Vec<AccountId>) -> Self {
        ...
        Self {
            ...
            token_imitation: LookupMap::new(b"a"),
        }
    }
    ...
    pub fn record_batch(&mut self, steps_batch: Vec<(AccountId, u32)>) {
        ...
        for (account_id, steps) in steps_batch.into_iter() {
            ...
            let sweat_to_mint: u128 = steps as u128;
            self.token.internal_deposit(&account_id, sweat_to_mint);
        }
    }

    pub fn record_batch_sim(&mut self, steps_batch: Vec<(AccountId, u32)>) {
        assert!(self.oracles.contains(&env::predecessor_account_id()));
        for (account_id, steps) in steps_batch.into_iter() {
            let sweat_to_mint: u128 = steps as u128;
            self.token_imitation.insert(&account_id, &sweat_to_mint);
        }
    }

    pub fn record_batch_austin(&mut self, steps_batch: Vec<(AccountId, u32)>) {
        assert!(self.oracles.contains(&env::predecessor_account_id()));
        for (account_id, steps) in steps_batch.into_iter() {
            let sweat_to_mint: u128 = steps as u128;
            internal_deposit(&mut self.token, &account_id, sweat_to_mint);
        }
    }
}
